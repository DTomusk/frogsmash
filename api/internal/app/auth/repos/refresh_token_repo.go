...
func (r *RefreshTokenRepo) SaveRefreshToken(token string, userID string, expiresAt int64, ctx context.Context, db shared.DBTX) error {
...
func (r *RefreshTokenRepo) RevokeTokens(userID string, ctx context.Context, db shared.DBTX) error {
	_, err := db.ExecContext(ctx, "UPDATE refresh_tokens SET revoked = TRUE WHERE user_id = $1", userID)
	return err
}

func (r *RefreshTokenRepo) GetRefreshToken(token string, ctx context.Context, db shared.DBTX) (*models.RefreshToken, error) {
